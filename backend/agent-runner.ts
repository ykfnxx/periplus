...
const kimiBin =
  process.env.PERIPLUS_KIMI_BIN ?? '/Users/yangkefan/.kimi-code/bin/kimi';

async function copyKimiIdentity(kimiHome: string) {
  const sourceHome =
    process.env.PERIPLUS_KIMI_HOME_SOURCE ?? join(homedir(), '.kimi-code');
...
export class AgentRunner {
  ...
  async start(sessionId: string, prompt: string, emit: AgentEventEmitter) {
    ...
    const child = spawn(kimiBin, ['-p', buildPrompt(conversationMessages)], {
      cwd: workDir,
      env: {
        ...process.env,
