...
- F1 Place resolution: decision state, city, coordinate system/error, provider
  identity, trace evidence for every required result status, parent-tool
  provenance, result-specific resolver allowlists, and exact ready-command
  target, coordinate, and provider bindings.
...
unsampled, append-only event stream with monotonic sequence numbers. The
optional `AgentGateway` trace integration records run, tool, command,
state-diff, and Place-evidence boundaries without copying authoritative
documents or provider payloads into events. Eval Trace is a deterministic
capability artifact, not a tamper-evident security audit log.
...
userinfo are redacted before persistence. `verifyEvalTrace` checks schema,
sequence, one root run, run/scenario identity, open-parent ordering, descendant
workspace/journey/turn identity, evidence provenance, command/state-diff
identity, cross-command revision continuity, idempotency-key terminal identity,
typed replay semantics, and lifecycle closure. A replay must match a prior
applied outcome and its original revisions exactly, and it cannot record a new
state diff. A rejected attempt preserves command identity for same-key conflict
detection but does not consume the idempotency key or become a replay baseline;
the same command may first apply after refreshing its expected revision.
...
Provider responses and full snapshots should live outside Eval Trace. Trace
events carry evidence references and typed lifecycle metadata, not raw image
bytes, credentials, environment values, or private source text.
