...
function autoPrompt() {
  return [
    ...
    "TOOL: periplus__workspace__get_context",
    "INPUT: {}",
    ...
    "TOOL: periplus__journey__project",
    ...
    "TOOLS: 可选 periplus__place__search；地点确认 periplus__place__resolve；可选图片 periplus__place__enrich；酒店推荐 periplus__hotel__search；仅 provider fallback 使用 WebSearch/web_search + periplus__place__fallback",
    "SAVE: placeResolutionIdByCardId, hotelSelectionIdByCardId",
    "BRANCH: resolved -> 保存 handle；ambiguous -> STAGE 10；not_found 且 fallbackAllowed=false -> STAGE 10；not_found 且 fallbackAllowed=true -> WebSearch 后调用 periplus__place__fallback；fallback_registered -> 保存 handle；图片 warning -> 继续；证据齐全 -> STAGE 05",
    ...
    "TOOLS: periplus__draft__add_city_card / periplus__draft__add_place_card / periplus__draft__add_hotel_stay_card / periplus__draft__add_place_stay_card / periplus__draft__add_transit_card；编辑时使用对应 update/move/remove 工具",
    ...
    "",
    "STAGE 07 VALIDATE",
    "TOOL: periplus__draft__validate",
    ...
    "PREPARE_TRANSIT: 调用 periplus__draft__prepare_transit(draftId, transitCardId, operationId)；不增加 repairsUsed；仍有未 READY Transit -> 重复 PREPARE_TRANSIT；出现非 Transit issue -> REPAIR；valid=true -> STAGE 09",
    "",
    "STAGE 09 COMMIT_AND_CONFIRM",
    "TOOL 1: periplus__draft__commit(draftId, 唯一 idempotencyKey)",
    "SAVE: newWorkspaceRevision",
    "TOOL 2: periplus__journey__validate_current(expectedWorkspaceRevision=newWorkspaceRevision)",
