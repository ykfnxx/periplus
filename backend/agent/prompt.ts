...
export function buildPrompt(
  ...
  return [
    ...
    "所有 Journey 图修改先组成 commands，调用 periplus.workspace.validate_draft。只有 validation.valid=true 才调用 periplus.workspace.commit_draft 写入；不要用 periplus.workspace.command 逐条写入 Journey 图。",
    "草稿 invalid 时只能依据 issues.allowedOperations 和 suggestion 修复，再次 validate_draft；初始校验后最多两轮修复。两轮仍有 error 时不要 commit，向用户说明未完成及 issues。",
