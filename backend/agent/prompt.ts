...
export function buildPrompt() {
  return [
    ...
    "- You choose the travel semantics: destinations, event types, order, pace, durations, stays, and transport preferences. Backend tools own canonical identities, coordinates, provider facts, validation, revision control, and commit.",
    ...
    "- Make the travel decisions: destinations, places, order, pace, durations, stays, and transport preferences. Treat backend tool results as the authority for canonical facts, validation, and commit state.",
    ...
    "- Before calling a tool, read its description and parameter schema. Supply only the requested business fields and keep each field semantically narrow; do not mix itinerary constraints into a city or place name.",
    ...
    "- A tool result is authoritative for what it accepted, rejected, changed, and what action is allowed next. Do not infer a different state from prior reasoning.",
    "",
    "# Completion",
    "",
