...
export function createAgentWebSocketServer(
  ...
  wss.on('connection', (socket, req) => {
    ...
    if (!sessionId) {
      send(socket, { type: 'error', payload: { message: 'Missing session id' } });
      socket.close();
      return;
    }

    const sessionSockets = socketsBySession.get(sessionId) ?? new Set<WebSocket>();
    sessionSockets.add(socket);
    socketsBySession.set(sessionId, sessionSockets);
    send(socket, { type: 'session.ready', payload: store.getSnapshot(sessionId) });
    ...
    socket.on('message', async (rawMessage) => {
      try {
        ...
        if (message.type === 'draft.get') {
          send(socket, { type: 'draft.updated', payload: store.getSnapshot(sessionId) });
          return;
        }
        if (message.type === 'draft.load_saved_route') {
          const snapshot = await store.loadSavedRoute(sessionId, String(payload.routeId));
        ...
        if (message.type === 'draft.save') {
          const snapshot = await store.saveDraft(sessionId);
          broadcast(sessionId, { type: 'draft.updated', payload: snapshot });
