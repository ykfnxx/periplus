...
const mcpAuthContext = { userId: "dev-admin", role: "admin" }
...
describe("route MCP tool handlers", () => {
  ...
  it("lists routes through the route service", async () => {
    ...
    expect(routeServiceMocks.listRoutes).toHaveBeenCalledWith(mcpAuthContext)
  ...
  it("adds route points with relative position", async () => {
    ...
    expect(routeServiceMocks.addRoutePoint).toHaveBeenCalledWith(
      mcpAuthContext,
      "route-1",
      {
        point: { name: "西湖", lat: 30.246, lng: 120.146 },
  ...
  it("moves route points through update_route_point", async () => {
    ...
    expect(routeServiceMocks.updateRoutePoint).toHaveBeenCalledWith(
      mcpAuthContext,
      "route-1",
      "point-1",
      {
