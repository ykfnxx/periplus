...
describe("route service", () => {
  ...
  it("creates routes through Prisma and returns a DTO", async () => {
    ...
    const route = await createRoute(userContext, {
      name: "杭州三日",
      points: [
        {
          name: "灵隐寺",
  ...
  it("returns null when a route is not found", async () => {
    prismaMocks.route.findFirst.mockResolvedValueOnce(null)

    await expect(getRoute(userContext, "missing-route")).resolves.toBeNull()
  })

  it("adds a route point after an anchor and normalizes order", async () => {
    prismaMocks.route.findFirst.mockResolvedValueOnce(baseRoute)
    prismaMocks.route.findUnique.mockResolvedValueOnce(routeAfterMutation)
    ...
    const route = await addRoutePoint(userContext, "route-1", {
      point: {
        name: "河坊街",
        lat: 30.242,
        lng: 120.171,
  ...
  it("returns null when adding a point to a missing route", async () => {
    ...
    await expect(
      addRoutePoint(userContext, "missing-route", {
        point: {
          name: "河坊街",
          lat: 30.242,
          lng: 120.171,
  ...
  it("rejects adding a route point relative to an anchor outside the route", async () => {
    ...
    await expect(
      addRoutePoint(userContext, "route-1", {
        point: {
          name: "河坊街",
          lat: 30.242,
          lng: 120.171,
  ...
  it("updates a route point patch and moves it before another point", async () => {
    prismaMocks.route.findFirst.mockResolvedValueOnce(baseRoute)
    prismaMocks.route.findUnique.mockResolvedValueOnce({
      ...baseRoute,
      points: [
    ...
    const route = await updateRoutePoint(userContext, "route-1", "point-1", {
      patch: { name: "灵隐寺飞来峰" },
  ...
  it("rejects positioning a route point relative to itself", async () => {
    ...
    await expect(
      updateRoutePoint(userContext, "route-1", "point-1", {
        patch: { name: "灵隐寺飞来峰" },
  ...
  it("deletes a route point and normalizes remaining order", async () => {
    ...
    const route = await deleteRoutePoint(userContext, "route-1", "point-1")
  ...
  it("rejects incomplete route point reorder lists", async () => {
    ...
    await expect(
      reorderRoutePoints(userContext, "route-1", ["point-1"])
    ).rejects.toBeInstanceOf(RouteInputError)
  })
  ...
  it("reorders route points from a complete unique list and normalizes order", async () => {
    ...
    const route = await reorderRoutePoints(userContext, "route-1", [
      "point-2",
      "point-1",
    ])
  ...
  it("derives temporary reorder orders above the current max order", async () => {
    ...
    prismaMocks.route.findFirst.mockResolvedValueOnce(highOrderRoute)
    prismaMocks.route.findUnique.mockResolvedValueOnce({
      ...baseRoute,
      points: [
        { ...baseRoute.points[1], order: 0 },
    ...
    await reorderRoutePoints(userContext, "route-1", ["point-2", "point-1"])
