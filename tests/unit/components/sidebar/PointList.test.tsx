...
describe('PointList', () => {
  ...
  it('shows empty message when no route is selected', () => {
    (useMapStore as unknown as ReturnType<typeof vi.fn>).mockImplementation((selector: (s: unknown) => unknown) => {
      const state = { currentRoute: null, selectedPoint: null, setSelectedPoint: mockSetSelectedPoint };
  ...
  it('shows empty message when route has no points', () => {
    (useMapStore as unknown as ReturnType<typeof vi.fn>).mockImplementation((selector: (s: unknown) => unknown) => {
      const state = { currentRoute: { points: [] }, selectedPoint: null, setSelectedPoint: mockSetSelectedPoint };
      return selector(state);
    });
  ...
  it('renders points in order', () => {
    ...
    (useMapStore as unknown as ReturnType<typeof vi.fn>).mockImplementation((selector: (s: unknown) => unknown) => {
      const state = { currentRoute: route, selectedPoint: null, setSelectedPoint: mockSetSelectedPoint };
      return selector(state);
    });
