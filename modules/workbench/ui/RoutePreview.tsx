...
function NodeItem({
  ...
  return (
    ...
    >
      {/* Timeline dot */}
      <div className="absolute top-1 -left-[18px] h-4 w-4 rounded-full border-[3px] border-white bg-[var(--periplus-russet)]" />
...
function EdgeItem({
  ...
  return (
    <div className="relative mb-5 w-full text-left">
      ...
      <div className="absolute top-1 -left-[16px] flex h-3 w-3 items-center justify-center rounded-full border-2 border-white bg-[var(--periplus-bluegray)]">
      ...
      >
        ...
        {edge.planningStatus === "PLANNING" && (
          <p className="mt-0.5 text-[11px] text-[var(--periplus-teak)]">
            正在规划真实路线
          </p>
        )}
        {edge.planningWarning && (
          <p className="mt-1 text-[11px] leading-4 text-[var(--periplus-coral)]">
            {edge.planningWarning}
          </p>
        )}
      </button>
          ...
          {edge.plans!.map((plan) => (
            <button
              ...
              className={`shrink-0 rounded-md border px-2 py-1 text-[10px] font-bold transition ${
                selected?.id === plan.id
                  ? "border-[var(--periplus-russet)] bg-[var(--periplus-russet)] text-white"
                  : "border-[rgb(44_36_22_/_14%)] bg-[var(--periplus-soft-white)] text-[var(--periplus-walnut)]"
              }`}
            >
              {plan.label} ·{" "}
  ...
  return (
    <div className="space-y-4 px-5 pt-4 pb-5">
      ...
      <div className="relative pl-7">
        {/* Vertical line */}
        <div className="absolute top-0 bottom-0 left-[10px] w-0.5 bg-[var(--periplus-bluegray)]" />
