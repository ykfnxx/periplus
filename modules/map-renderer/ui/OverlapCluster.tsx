...
function shouldUseDarkMarkerText(color: string) {
  return (
    (routeMarkerColors as readonly string[]).includes(color) ||
    color === periplusColors.mustard ||
    color === periplusColors.bluegray
  )
}
...
function createClusterIcon(
  ...
  cluster.anchors.slice(0, 2).forEach((anchor, index) => {
    const icon = createAnchorIcon(anchor, routeNodeMeta, photoShares, "preview")
...
export default function OverlapCluster({ onIntent }: OverlapClusterProps) {
  ...
  const anchorCluster = useWorkspaceStore((s) => s.anchorCluster)
  const expandCluster = useWorkspaceStore((s) => s.expandCluster)
  const collapseCluster = useWorkspaceStore((s) => s.collapseCluster)
  ...
  const clusters = useMemo<ClusterGroup[]>(() => {
    ...
    return calculateAnchorClusters(anchors, (anchor) =>
  ...
  const updateRouteNodeMeta = useCallback(() => {
    ...
    if (draftRoute) {
      const view = getActivePathView(draftRoute, viewLevel, activeRouteNodeId)
      const sortedNodes = [...view.nodes].sort((a, b) => a.order - b.order)
  ...
  }, [draftRoute, viewLevel, activeRouteNodeId])
    ...
    anchorCluster.scatteredAnchors.forEach((anchor) => {
      ...
      marker.on("click", (e) => {
        ...
        if (anchor.type === "route") {
          ...
          const point = view.nodes.find((p) => `route-${p.id}` === anchor.id)
          if (point) {
            onIntent?.({
              type: "map.waypoint-selected",
              waypointId: point.id,
              anchor: lngLatToAnchor(scatteredLngLat),
            })
          }
        } else {
          const photo = photoShares.find((p) => `photo-${p.id}` === anchor.id)
